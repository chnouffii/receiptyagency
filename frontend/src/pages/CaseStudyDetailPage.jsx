...
export default function CaseStudyDetailPage() {
  ...
  if (loading) {
    return <div className="pt-24 bg-[#050505] min-h-screen flex items-center justify-center"><p className="text-gray-500">{lang === 'fr' ? 'Chargement...' : 'Loading...'}</p></div>;
  }

  if (!cs) {
    return <div className="pt-24 bg-[#050505] min-h-screen flex items-center justify-center"><p className="text-gray-400">{lang === 'fr' ? 'Etude de cas introuvable.' : 'Case study not found.'}</p></div>;
  }
    ...
    <div data-testid="case-detail-page" className="pt-24 bg-[#050505] min-h-screen">
      <div className="max-w-4xl mx-auto px-6 py-16">
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }}>
          ...
          <h1 className="font-heading text-3xl sm:text-4xl lg:text-5xl font-bold text-white tracking-tight leading-tight">{title}</h1>
          <p className="mt-4 text-base text-gray-400 leading-relaxed max-w-2xl">{desc}</p>
        </motion.div>

        {cs.image_url && (
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2, duration: 0.5 }} className="mt-10 rounded-2xl overflow-hidden border border-white/5">
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3, duration: 0.5 }} className="mt-8 grid grid-cols-3 gap-4">
            {duration && (
              <div className="rounded-xl border border-white/5 bg-[#0F0F10] p-5">
                ...
                <p className="mt-1 font-mono text-lg font-bold text-white">{duration}</p>
              </div>
            )}
            {team && (
              <div className="rounded-xl border border-white/5 bg-[#0F0F10] p-5">
                <p className="text-xs text-gray-600 uppercase tracking-wider">{sectionLabel.team}</p>
                <p className="mt-1 font-mono text-lg font-bold text-white">{team}</p>
              </div>
            )}
            <div className="rounded-xl border border-white/5 bg-[#0F0F10] p-5">
        ...
        {challenge && (
          <motion.section initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.4, duration: 0.5 }} className="mt-12">
            ...
            <p className="text-gray-400 leading-relaxed pl-8">{challenge}</p>
          </motion.section>
        )}
        ...
        {solution && (
          <motion.section initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.5, duration: 0.5 }} className="mt-10">
            ...
            <p className="text-gray-400 leading-relaxed pl-8">{solution}</p>
          </motion.section>
        )}
